...
The responsive Jerusalem hero, transparent logo, mosaic illustrations, Give page artwork, split card backgrounds, first visit card artwork, mobile building artwork, and social card came from assets supplied or approved during the redesign and are stored directly under `public/images/`. `hero-desktop-v3.jpg` is the deterministic 2x upscale of the owner-supplied desktop image added on August 15, 2026; `hero-desktop-v2.jpg` is the original 1280x720 upload and `hero-desktop.jpg` remains the prior rollback asset. The six `card-*.webp` files preserve the exact supplied artwork: lower focal crops support Prayer, Proclamation, and People on the homepage, while the abstract upper crops support the Three P’s on the Beliefs page. The three `expect-*.webp` files support Joyful Worship, Rooted Teaching, and Room for Family in the order supplied. The three `ministry-*-group.webp` files are landscape focal crops of the supplied artwork for the Children, Young Adults, and Women & Men ministry cards. `give-banner.webp` is the supplied wide mosaic artwork used above the Give page content. `building-mobile.webp` is the user supplied square composition that keeps the full façade, entrance, address, and TAJ sign legible on phones. `apple-touch-icon.png` is derived from the supplied transparent logo on the site’s navy background.

The versioned mobile hero media was supplied by the site owner on July 26, 2026 as a motion treatment derived from the approved Jerusalem mobile hero artwork. The silent H.264 MP4 and VP9 WebM source assets are stored under `public/_hero-media/` and served through the versioned `/videos/hero/` range endpoint; the matching WebP first frame remains a direct static asset. The original emailed `Video.mov` remains outside the public build.
