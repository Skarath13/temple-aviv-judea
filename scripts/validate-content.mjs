...
const readMdxFrontmatter = async (filename) => {
	...
	try {
		...
		if (!match) {
			report(
				`${relativePath} must contain only YAML frontmatter and an empty MDX body.`,
			);
			return null;
		}
...
const validateContentTree = (value, label) => {
	if (Array.isArray(value)) {
		value.forEach((item, index) =>
			validateContentTree(item, `${label}[${index}]`),
		);
		return;
	}
...
const validateIdentityList = (items, label, expectedValues) => {
	...
	if (
		...
		report(
			`${label} must contain exactly one of each: ${expectedValues.join(", ")}.`,
		);
	}
};
...
for (const [filename, expectedRoute] of expectedPages) {
	...
	if (page._template !== pageKey)
		report(`${filename}._template must remain ${pageKey}.`);
	if (page.route !== expectedRoute)
		report(`${filename}.route must remain ${expectedRoute}.`);
...
if (site) {
	...
	for (const [label, value, hosts] of providerLinks) {
		if (!isAllowedSecureUrl(value, hosts))
			report(`site.${label} is not an approved provider URL.`);
	}
	if (!isApprovedGivingUrl(site.giving)) {
		report(
			"site.giving must remain the approved PayPal recipient and checkout URL.",
		);
	}
	if (!isEmailAddress(site.email))
		report("site.email must be a valid email address.");
	if (!isTelephoneLink(site.phoneHref))
		report("site.phoneHref must be an E.164 tel: link.");
	try {
	...
	for (const [index, item] of (site.navigation || []).entries()) {
		if (!isRootRelativePath(item?.href) || !publicRoutes.has(item.href)) {
			report(
				`site.navigation[${index}].href must target a retained public page.`,
			);
		}
		if (navigationRoutes.has(item?.href))
			report(`site.navigation duplicates ${item.href}.`);
