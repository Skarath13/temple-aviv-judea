...
for (const asset of assets) {
  const [contents, details] = await Promise.all([readFile(asset.path), stat(asset.path)]);
  const sha256 = createHash('sha256').update(contents).digest('hex');
  if (details.size !== asset.bytes || sha256 !== asset.sha256) {
    throw new Error(`${asset.path} no longer matches its versioned byte and SHA-256 contract.`);
  }
  if (details.size >= MAX_STATIC_ASSET_BYTES) {
    throw new Error(`${asset.path} exceeds the Workers Static Assets file limit.`);
  }
}
