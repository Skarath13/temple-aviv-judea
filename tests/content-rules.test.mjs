...
test("accepts only clean root-relative site paths", () => {
	const valid = ["/", "/visit/", "/beliefs/#prayer", "/search/?query=shabbat"];
	const invalid = [
		"",
		"visit/",
...
test("email links allow safe subject/body text but reject header injection", () => {
	...
	assert.equal(
		isSafeCmsLink(
			"mailto:editor@example.com?subject=Hello%0ABcc:attacker@example.com",
		),
		false,
	);
});
...
test("the donation destination cannot be changed to another PayPal account", () => {
	...
	assert.equal(
		isApprovedGivingUrl(
			"https://www.paypal.com/donate/?business=attacker%40example.com",
		),
		false,
	);
});
...
test("editor validators accept empty optional values and explain invalid values", () => {
	...
	assert.equal(
		validateAllowedHost("", allowedSiteHosts.youtube, "YouTube"),
		undefined,
	);
