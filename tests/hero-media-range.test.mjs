...
test("returns 416 for invalid, multipart, and unsatisfiable ranges", async () => {
	...
	for (const range of headers) {
		...
		assert.equal(response.headers.get("Content-Range"), `bytes */${MP4_BYTES}`);
...
test("serves ranged HEAD metadata without a body", async () => {
	...
	assert.equal(response.headers.get("Content-Range"), `bytes 0-1/${MP4_BYTES}`);
	assert.equal(harness.assetFetcher.calls.length, 0);
	assert.equal(harness.cacheStorage.openedNames.length, 0);
});
...
test("uses a native ranged cache hit without fetching the origin", async () => {
	...
	assert.equal(harness.assetFetcher.calls.length, 1);
	assert.deepEqual(body, harness.bytes.subarray(2_919_987));
	assert.ok(
		harness.cacheStorage.cache.matchRequests.some(
			(candidate) =>
				candidate.headers.get("Range") === `bytes=2919987-${MP4_BYTES - 1}`,
		),
	);
});
...
test("cancels the source stream after completing a probe range", async () => {
	...
	const stream = createRangeStream(source, { end: 1, start: 0 }, context);
	...
	assert.ok(cancellation.reasons.includes("hero-media-range-complete"));
});
