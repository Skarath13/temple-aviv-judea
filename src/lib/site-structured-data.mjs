...
const publicUrl = (value, label, { httpsOnly = false } = {}) => {
	const url =
		value instanceof URL ? new URL(value) : new URL(requiredText(value, label));
	...
	if (
		...
		throw new TypeError(
			`${label} must be ${protocolLabel} URL without credentials.`,
		);
	}
...
export const normalizePublicHours = (hours) => {
	if (
		!Array.isArray(hours) ||
		hours.length === 0 ||
		hours.length > DAY_ORDER.length
	) {
		throw new TypeError(
			"site.publicHours must contain between 1 and 7 entries.",
		);
	}
	...
	const normalized = hours.map((item, index) => {
		...
		const closes = requiredText(
			item.closes,
			`site.publicHours[${index}].closes`,
		);

		if (!DAY_INDEX.has(dayOfWeek)) {
			throw new TypeError(
				`site.publicHours[${index}].dayOfWeek is not a valid weekday.`,
			);
		}
		if (days.has(dayOfWeek)) {
			throw new TypeError(
				`site.publicHours contains duplicate day ${dayOfWeek}.`,
			);
		}
		...
		if (opens >= closes) {
			throw new TypeError(
				`site.publicHours[${index}].opens must be before closes.`,
			);
		}
	...
	return normalized.sort(
		(left, right) =>
			DAY_INDEX.get(left.dayOfWeek) - DAY_INDEX.get(right.dayOfWeek),
	);
};
...
export const createSiteStructuredData = ({ site, siteRoot, imageUrl }) => {
	...
	const maps = publicUrl(site.address?.maps, "site.address.maps", {
		httpsOnly: true,
	});
	const sameAs = [
	...
	if (!/^[A-Z]{2}$/.test(country)) {
		throw new TypeError(
			"site.address.country must be a two-letter uppercase country code.",
		);
	}
