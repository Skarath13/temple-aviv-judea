...
const mediaContracts = new Map([
  ...
  [
    '/videos/hero/mobile-jerusalem-v1.webm',
    {
      ...
      etag:
        '"68f4fca47ca0e38300dcee62d5f372c23ea9c08e1e6bbd44279e3d56d9509e68"',
...
const createErrorResponse = (status, message, contract, cacheStatus) => {
  ...
  headers.set('Content-Length', String(new TextEncoder().encode(message).length));
  ...
  return new ReadableStream({
    ...
    async pull(controller) {
      while (true) {
        ...
        if (done) {
          controller.error(new Error('Hero media ended before the requested range.'));
          return;
        }
