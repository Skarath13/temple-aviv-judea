...
const normalizeSiteRoot = (value) => {
	...
	if (
		...
		throw new TypeError(
			"siteRoot must be a public HTTP(S) URL without credentials, a query, or a fragment.",
		);
	}
...
const publicPageUrl = (siteRoot, route) => {
	...
	if (
		...
		throw new TypeError(
			`Public page route escaped the configured site root: ${route}`,
		);
	}
...
const pageSort = (left, right) => {
	const leftPriority =
		ROUTE_PRIORITY.get(left.route) ?? Number.MAX_SAFE_INTEGER;
	const rightPriority =
		ROUTE_PRIORITY.get(right.route) ?? Number.MAX_SAFE_INTEGER;
